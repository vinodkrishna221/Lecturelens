...
| **On-Device AI — SLM** | Phi-4-mini (3.8B, Q4_K_M) via `llama.cpp` React Native bridge | Best instruction-following at this size; high-throughput inference on Snapdragon 8 Gen 5. Note structuring + quiz generation stays on-device. Unchanged from v1. | Cloud LLM (GPT-4o/Gemini) — adds cost, latency, privacy concerns |
